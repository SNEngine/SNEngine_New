...
public sealed class StatementParser
{
    ...
    private readonly NativeBlockParser _nativeBlockParser;   // Native support
    ...
    public StatementParser(Parser<char, CommandNode> commandParser,
                           ...
                           NativeBlockParser nativeBlockParser)
    {
        ...
        _nativeBlockParser = nativeBlockParser ?? throw new ArgumentNullException(nameof(nativeBlockParser));
    }
    ...
    public CommandNode? ParseNext(TokenReader reader)
    {
        ...
        // ... (все существующие проверки break/continue/return/local остаются без изменений) ...
        ...
        // === Native block (high priority) ===
        if (lineContent.StartsWith("native", StringComparison.OrdinalIgnoreCase))
        {
            return _nativeBlockParser.Parse(reader);
        }
